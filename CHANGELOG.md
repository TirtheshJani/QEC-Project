...
## Current status
Phase 0 complete (foundations: linear algebra, probability + BSC, classical EC with Hamming(7,4) over BSC). Next: Phase 1 (quantum basics: states, gates, measurement, density matrices, Bloch sphere) is being built in parallel in a sibling worktree; coordinate at merge time. After Phase 1: Phase 2 (QEC fundamentals; stabilizer codes; Steane reuses Hamming(7,4) helpers from this phase). Commit locally on the active branch, do not push without explicit ask.
