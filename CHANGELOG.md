...
## Current status
Phase 1 complete (1.1 qubits-and-gates + 1.2 multi-qubit/Bell/superdense/teleportation + 1.3 density matrices and Kraus channels). Phase 0.3 (Hamming(7,4) over BSC) was in flight in a parallel worktree on the same day; final integration status TBD by orchestrator merge. Next: `phase-02-qec-fundamentals/` (stabilizer formalism, Pauli group machinery, three-qubit bit-flip and phase-flip repetition codes). Branch `main` after merge; commit locally, do not push without explicit ask.
